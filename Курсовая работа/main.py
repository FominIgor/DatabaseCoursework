...
INNER JOIN СтатусДокумента ON Документы.IDdocumentstatus = СтатусДокумента.IDdocumentstatus; """)):
    ...
    def insert_staff(self):
        ...
        try:
            ...
            '{id_author}', '{id_typdoc}', '{id_tytstatusdoc}', 
    ...
    def appdate(self):
        ...
        query.prepare("""UPDATE Документы SET DocumentNumber=:number, 
        Dateofcreation=:datestart, IDauthor=:id_author, 
        IDTypeofDocument=:id_typdoc, IDdocumentstatus=:id_tytstatusdoc, 
...
INNER JOIN Отделы ON Сотрудники.IDofthedepartment = Отделы.IDofthedepartment; """)):
    ...
    def apdate_slot(self):
        app = QSqlDatabase.addDatabase("QSQLITE")
        app.setDatabaseName("database coursework.db")
        app.open()
        ...
        # выполнить обновление данных в таблице Документы, используя id
        query.prepare("""UPDATE Сотрудники SET Fullname=:Fullname, IDposition=:IDposition, IDofthedepartment=:IDofthedepartment, Thedateofemployment=:Thedateofemployment, Telephone=:Telephone, Email=:Email WHERE IDemployee=:id""")
        query.bindValue(":Fullname", self.fio.text())
        query.bindValue(":IDposition", id_dolgnost)
        query.bindValue(":IDofthedepartment", id_otdel)
        query.bindValue(":Thedateofemployment", self.datestart.text())
        query.bindValue(":Telephone", self.phone.text())
        query.bindValue(":Email", self.mail.text())
        query.bindValue(":id", self.id.text())
        query.exec_()
        app.commit()
        app.close()
...
INNER JOIN Сотрудники ON Отделы.SotrokdnikIDBrutder = Сотрудники.IDemployee; """)):
    ...
    def apdate_slot(self):
        ...
        appt = QSqlDatabase.addDatabase("QSQLITE")
        appt.setDatabaseName("database coursework.db")
        appt.open()

        typdoc = self.king.currentText()
        query = QSqlQuery(appt) 
        query.prepare("SELECT IDemployee FROM Сотрудники WHERE Fullname = :name")
        query.bindValue(":name", typdoc)
        query.exec_()
        query.next()
        ...
        # выполнить обновление данных в таблице Документы, используя id
        query.prepare("""UPDATE Отделы SET Thenameofthedepartment=:Thenameofthedepartment, SotrokdnikIDBrutder=:SotrokdnikIDBrutder WHERE IDofthedepartment=:id""")
        query.bindValue(":Thenameofthedepartment", self.name.text())
        query.bindValue(":SotrokdnikIDBrutder", id_king)
        query.bindValue(":id", self.id.text())
        query.exec_()   
        appt.commit()
        appt.close()
...
INNER JOIN Сотрудники ON ИсполнителиДокументов.IDemployee = Сотрудники.IDemployee;
 """):
    ...
    def apdate_slot(self):
        ...
        # выполнить обновление данных в таблице Документы, используя id
        query.prepare("""UPDATE СвязанныеДокументы SET IDdocumentSource=:IDdocumentSource, IDofthedocumentRelated=:IDofthedocumentRelated WHERE IDrelateddocuments=:id""")
        query.bindValue(":IDdocumentSource", docs2)
        query.bindValue(":IDofthedocumentRelated", docs3)
        query.bindValue(":id", self.id.text())
        query.exec_()   
        appt.commit()
        appt.close()
    ...
    def apdate_slot2(self):
        ...
        # выполнить обновление данных в таблице Документы, используя id
        query.prepare("""UPDATE ИсполнителиДокументов SET IDdocument=:IDdocument, IDemployee=:IDemployee WHERE IDexecutorsofdocuments=:id""")
        query.bindValue(":IDdocument", docs2)
        query.bindValue(":IDemployee", sotrudnic)
        query.bindValue(":id", self.id_2.text())
        query.exec_()   
        appt.commit()
        appt.close()
        self.update_twStaffs2()
